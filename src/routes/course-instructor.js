    ...
    if (!req.body) {
        ...
        newCourse.save()
            .then(course => {
                ...
                newStudentList.save()
                    .then(studentList => {
                        courseModel.updateOne({ "_id": objectId(course.id) }, {
                            $set: {
                                    ...
                                    if (user) {
                                        scheduleModel.findOne({ "_id": objectId(user.schedulechosen) })
                                            .then(schedule => {
                                                ...
                                                scheduleModel.updateOne({ "_id": objectId(schedule.id) }, {
                                                    $set: {
...
router.get('/view-courses-schedule', ensureAuthenticated, (req, res) => {
    res.render("viewCourseSchedule-instructor");
})

router.get('/view-upcoming-courses-schedule', ensureAuthenticated, (req, res) => {
    const monthsArray = [
        'January',
        'February',
        'March',
                ...
                function getCourses(courseArray, index) {
                    if (typeof courseArray[index] != "undefined") {
                        ...
                        courseModel.findOne({ "_id": objectId(courseArray[index]) })
                            .then(course => {
                                if (course) {
                                    studentListModel.findOne({ "_id": objectId(course.studentslistId) })
                                        .then(studentList => {
                                            if (studentList) {
                                                var courseCopy = course;
                                                courseCopy.sessioninterval.push(studentList.studentslist);
                                                courseContentList.push(courseCopy);
                        ...
                        res.json({ courses: courseFilteredList, months: monthsArray })
                    }
                }
            }
        })
